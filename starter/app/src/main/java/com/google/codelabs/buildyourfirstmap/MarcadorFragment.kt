...
import com.google.codelabs.buildyourfirstmap.LoginActivity
...
class MarcadorFragment : Fragment() {
        ...
        db.collection("users").whereEqualTo("uId", userName).get()
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    _binding.currencyOne.text = document.data["currencyOne"].toString()
    ...
    private fun checkCurrencyTwo(userName: String) {
        ...
        db.collection("users").whereEqualTo("uId", userName).get()
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    _binding.currencyTwo.text = document.data["currencyTwo"].toString()
